...
export default function VideoCard({ video, onPlay, onYouTubeUpload }) {
  ...
  const isPostedOnYouTube = Boolean(video.youtube_url);

  const handleYouTubeUpload = async () => {
    if (!onYouTubeUpload || isPostedOnYouTube) return;
    setUploading(true);
    try {
        ...
        <div className="flex gap-2 mt-2">
          ...
          {isPostedOnYouTube ? (
            <a
              ...
              className="flex-1 text-center py-2 bg-green-600/20 text-green-400 text-sm font-medium rounded-lg hover:bg-green-600/30 transition-colors"
              title={`Watch on YouTube: ${video.youtube_url}`}
            >
              ✓ Posted on YouTube
            </a>
          ) : (
